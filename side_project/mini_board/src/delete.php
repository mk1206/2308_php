...
try {
	...
	if($http_method === "GET") {
		...
		$conn->commit(); // commit
		header("Location: list.php"); // list 페이지로 이동
		exit; // 처리 종료
	}
} catch(Exception $e) {
	// POST일 경우에만 rollback
...
<body>
	...
	<section>
		<form action="/mini_board/src/delete.php" method="post">
			...
			<button class="delete-btn" type="submit">동의</button>
			<a class="delete-btn" href="/mini_board/src/detail.php/?id=<?php echo $id; ?>&page=<?php echo $page; ?>">취소</a>
		</form>
	</section>
</body>
</html> 